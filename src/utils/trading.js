...
function normalizeQuantity(rawQuantity, exchangeInfo) {
  ...
  if (!lotSize) {
    return {
      ...
      quantityNumber: Number(rawQuantity),
      lotSize: null,
    };
  }
...
function validateNotional(price, quantityNumber, exchangeInfo) {
  const notionalFilter =
    getFilter(exchangeInfo, "NOTIONAL") || getFilter(exchangeInfo, "MIN_NOTIONAL");

  const notional = Number(price) * Number(quantityNumber);
  const minNotional = notionalFilter
    ? Number(notionalFilter.minNotional || notionalFilter.notional || 0)
    : 0;
